...
hdd_data_autauga = hdd_data[hdd_data["FIPS"] == 1001]
with open("log.txt", "w") as f:
    f.write(hdd_data_autauga.to_string())

hdd_data_autauga_jan = hdd_data_autauga.drop(
    MONTHS[1:] + ["Code", "FIPS", "Year"], axis=1
)
with open("log.txt", "w") as f:
    f.write(hdd_data_autauga_jan.to_string())

# region forecast
hdd_data_autauga_jan["Lag_1"] = hdd_data_autauga_jan["Jan"].shift(1)

X = hdd_data_autauga_jan.loc[:, ["Lag_1"]]
X.dropna(inplace=True)  # drop missing values in the feature set
y = hdd_data_autauga_jan.loc[:, "Jan"]  # create the target
