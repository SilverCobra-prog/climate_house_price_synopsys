...
tavg_data_autauga = tavg_data[tavg_data["FIPS"] == 48039]
with open("log.txt", "w") as f:
    f.write(tavg_data_autauga.to_string())

tavg_data_autauga_jan = tavg_data_autauga.drop(
    MONTHS[:1] + MONTHS[2:] + ["Code", "FIPS", "Year"], axis=1
)
with open("log.txt", "w") as f:
    f.write(tavg_data_autauga_jan.to_string())

# region forecast
tavg_data_autauga_jan["Lag_1"] = tavg_data_autauga_jan["Feb"].shift(1)

X = tavg_data_autauga_jan.loc[:, ["Lag_1"]]
X.dropna(inplace=True)  # drop missing values in the feature set
y = tavg_data_autauga_jan.loc[:, "Feb"]  # create the target
