...
# create array to hold the total growth prediction for each county
growth_predictions = []
...
demographic_data["noaa_state_code"] = demographic_data["State"].map(
    noaa_state_codes
)
demographic_data["CountyId"] = (
    demographic_data["CountyId"].astype("str").apply(lambda x: x[-3:])
)
demographic_data["CountyId"] = (
    (demographic_data["noaa_state_code"] + demographic_data["CountyId"])
    .dropna()
    .astype("int")
)
demographic_data = demographic_data.rename({"CountyId": "fips"}, axis=1)

for fips in tqdm(full_data["fips"].unique()[:]):
    full_data_county = full_climate_data[full_climate_data["FIPS"] == fips]
    ...
    growth_predictions.append(
        (demographic_data[demographic_data["fips"] == fips], total_growth)
    )
...
national_house_inflation = result_data["Inflation Rate"].iloc[-1]

# endregion

np.save("growth_predictions.npy", growth_predictions)

# plt.scatter(
#     [x[0] for x in growth_predictions],
#     [x[1] for x in growth_predictions],
# )
# plt.show()
