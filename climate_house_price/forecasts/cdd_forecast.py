...
cdd_data_autauga = cdd_data[cdd_data["FIPS"] == 1001]
with open("log.txt", "w") as f:
    f.write(cdd_data_autauga.to_string())

cdd_data_autauga = cdd_data_autauga[cdd_data_autauga["Year"] !=2023]
cdd_data_autauga_jan = cdd_data_autauga.drop(
    MONTHS[:3] + MONTHS[4:] + ["Code", "FIPS", "Year"], axis=1
)
with open("log.txt", "w") as f:
    f.write(cdd_data_autauga_jan.to_string())

# region forecast
cdd_data_autauga_jan["Lag_1"] = cdd_data_autauga_jan["Apr"].shift(1)

X = cdd_data_autauga_jan.loc[:, ["Lag_1"]]
X.dropna(inplace=True)  # drop missing values in the feature set
y = cdd_data_autauga_jan.loc[:, "Apr"]  # create the target
